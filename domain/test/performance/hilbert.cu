...
int main()
{
...

    {
        ...
        auto computeHilbert = [&]()
        { computeSfcKeys(execution::Gpu{0}, rawPtr(dx), rawPtr(dy), rawPtr(dz), rawPtr(hilbertKeys2), numKeys, box); };

        auto computeMorton = [&]()
        { computeSfcKeys(execution::Gpu{0}, rawPtr(dx), rawPtr(dy), rawPtr(dz), rawPtr(mortonKeys2), numKeys, box); };
