...
void surfaceDetection()
{
    ...
    {
        treeBoxes[i] = sfcIBox(sfcKey(fullTree.codeStart(i)), fullTree.level(i), uniformAxesBits<KeyType>);
    }
...
void dualTraversalNeighbors()
{
    ...
    auto crossFocusSurfacePairs = [focusStart, focusEnd, &tree = octree, &box](TreeNodeIndex a, TreeNodeIndex b)
    {
        ...
        IBox aBox = sfcIBox(sfcKey(tree.codeStart(a)), tree.level(a), uniformAxesBits<KeyType>);
        IBox bBox = sfcIBox(sfcKey(tree.codeStart(b)), tree.level(b), uniformAxesBits<KeyType>);
    ...
    for (auto p : pairs)
    {
        ...
        IBox aBox = sfcIBox(sfcKey(octree.codeStart(a)), octree.level(a), uniformAxesBits<KeyType>);
        IBox bBox = sfcIBox(sfcKey(octree.codeStart(b)), octree.level(b), uniformAxesBits<KeyType>);
