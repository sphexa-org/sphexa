...
TEST(GeneralFocusExchangeGpu, bareTreelet)
{
    ...
    copy(gatherMaps, d_gatherMaps, Execution<GpuTag>{0});
    ...
    exchangeTreeletGeneral<unsigned>(peers, peers, d_gatherMapsView,
                                     ...
                                     0, scratch, MPI_COMM_WORLD, Execution<GpuTag>{0});
...
static void generalExchangeRandomGaussian(int thisRank, int numRanks)
{
    ...
    FocusedOctree<KeyType, T, GpuTag> focusTree(thisRank, numRanks, bucketSizeLocal, MPI_COMM_WORLD, Execution<GpuTag>{0});
