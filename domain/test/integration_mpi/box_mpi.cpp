...
void makeGlobalBox(int rank, int numRanks)
{
    ...
    Box<T> box = makeGlobalBox(x.data(), y.data(), z.data(), x.size(), Box<T>{0, 1}, MPI_COMM_WORLD);
    ...
    // PBC case
    {
        ...
        Box<T> newPbcBox = makeGlobalBox(x.data(), y.data(), z.data(), x.size(), pbcBox, MPI_COMM_WORLD);
