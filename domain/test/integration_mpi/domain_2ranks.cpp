...
void noHalos(int rank, int numRanks)
{
    int bucketSize = 1;
    float theta    = 1.0;
    Domain<KeyType, T> domain(rank, numRanks, bucketSize, bucketSize, theta, MPI_COMM_WORLD, Stream<CpuTag>{});
...
void multiStepSync(int rank, int numRanks)
{
    ...
    float theta         = 1.0;
    Domain<KeyType, T> domain(rank, numRanks, bucketSize, bucketSizeFocus, theta, MPI_COMM_WORLD, Stream<CpuTag>{});
...
void domainHaloRadii(int rank, int nRanks)
{
    ...
    int bucketSizeFocus = 1;
    float theta         = 1.0;
    Domain<KeyType, T> domain(rank, nRanks, bucketSize, bucketSizeFocus, theta, MPI_COMM_WORLD, Stream<CpuTag>{});
