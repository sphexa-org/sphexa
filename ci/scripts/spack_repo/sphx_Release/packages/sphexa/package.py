...
class Sphexa(CMakePackage, CudaPackage, ROCmPackage):
    ...
    version(
        "0.96.2",
        tag="v0.96.2",
        commit="03cc0ca2e770098430677b4a6613e35b6cab793d",
        submodules=True,
    )
    version(
        "0.95",
        tag="v0.95",
        commit="5e12d2926065cf376de30e8cfd082c4bfed78000",
        submodules=True,
    )
    version(
        "0.93.1",
        tag="v0.93.1",
        commit="4be3f10a95f260d4e3fb901e98a8336ac6af2478",
        submodules=True,
    )
    version("develop", branch="develop", submodules=True)

    variant("overlap", default=False, description="Enable custom srun flags")
    ...
    variant(
        "grackle", default=False, description="Enable radiative cooling with GRACKLE"
    )
    ...
    def cmake_args(self):
        ...
        if spec.satisfies("+rocm") or spec.satisfies("+cuda"):
            args.append(
                self.define_from_variant("CSTONE_WITH_GPU_AWARE_MPI", "gpu_aware_mpi")
            )
        ...
        if spec.satisfies("+cuda"):
            args.append(
                self.define("CMAKE_CUDA_FLAGS", f'-ccbin={spec["mpi"].mpicxx}')
            )
