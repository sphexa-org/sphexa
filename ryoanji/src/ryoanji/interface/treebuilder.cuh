...
public:
    ...
    int update(T* x, T* y, T* z, size_t numBodies, const cstone::Box<T>& box)
    {
        ...
        cstone::sequence(cstone::execution::Gpu{0}, rawPtr(d_ordering), d_ordering.size(), 0);
        cstone::sortByKey(cstone::execution::Gpu{0}, rawPtr(d_keys), rawPtr(d_keys) + d_keys.size(),
        ...
        cstone::fill(cstone::execution::Gpu{0}, rawPtr(d_layout_), rawPtr(d_layout_) + 1, LocalIndex(0));
        cstone::inclusiveScan(cstone::execution::Gpu{0}, rawPtr(d_counts_), rawPtr(d_counts_) + d_counts_.size(),
                                 rawPtr(d_layout_) + 1);
